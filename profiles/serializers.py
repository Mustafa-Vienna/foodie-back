...
class CustomTokenObtainPairSerializer(TokenObtainPairSerializer):
    def validate(self, attrs):
        ...
        if profile_image:
            profile_image_url = (
                ...
                if hasattr(profile_image, "url")
                    ...
                    else "https://res.cloudinary.com/duemxeswe/image/upload/"
                    + str(profile_image)
                )
            )
        else:
            profile_image_url = (
                "https://res.cloudinary.com/duemxeswe/image/upload/"
                "v1737306346/default_profile_girwrs.jpg"
            )
        ...
        data["user"] = {
            ...
            "email": user.email,
            "profile_id": (
                user.profile.id if hasattr(user, "profile") else None
            ),
