...
class PostSerializer(serializers.ModelSerializer):
    ...
    category = serializers.ChoiceField(
        choices=Post.CATEGORY_CHOICES, default='others')
    image_filter = serializers.ChoiceField(
        choices=Post.IMAGE_FILTER_CHOICES, default='normal')
    ...
    def validate_image(self, value):
        ...
        if value and (value.image.height > 4096 or value.image.width > 4096):
            raise serializers.ValidationError(
                "Image dimensions cannot exceed 4096x4096px.")
        return value
    ...
    def validate_content(self, value):
        ...
        if not all(key in value for key in required_keys):
            raise serializers.ValidationError(
                "Content must include introduction,"
                "ingredients, steps, and conclusion.")
        return value
